...
* On Linux (`linux-64`): GCC: (Anaconda package `gcc`)
* On Mac (`osx-arm64`): Clang: (Anaconda package `clang==18.1.8`)
...
If the following troubleshooting notes do not help, try the EMEWS mailing list:
<<_emews_mailing_list,EMEWS Mailing List>>

=== Problems with R

==== Compile-time problems with R

If R packages fail to install, check the following settings that affect how R compiles its packages from C/C++ source code.

1. Ensure no unnecessary environment variables are set, particularly `R_LIBS_USER`.  See here for more information about environment variables that affect R: +
...
4. Check your R build configuration files, `~/R/Makevars` and `~/.Renviron` .  These should normally be empty for EMEWS, however, you can re-add any needed features after you get EMEWS working.

5. To temporarily deactivate `Makevars`, set these variables before running the EMEWS install script:
+
[source,bash]
----
$ export R_MAKEVARS_USER='' R_MAKEVARS_SITE=''
